...
enum QrGenerator {
    static func make(text: String, side: CGFloat, logo: UIImage?) -> UIImage? {
        ...
        let out = max(1, side)
        let intScale = max(1, floor(out / qrSide))
        ...
        return renderer.image { r in
            ...
            UIBezierPath(rect: CGRect(x: 0, y: 0, width: out, height: out)).fill()

            UIImage(cgImage: cg).draw(in: CGRect(x: 0, y: 0, width: out, height: out))
            ...
            let desiredW = out * 0.28
            let maxLogoW = min(desiredW, pxW)
            let scaleFactor = maxLogoW / max(pxW, 1)
            let drawW = maxLogoW
            let drawH = pxH * scaleFactor
            let logoFrame = CGRect(x: (out - drawW)/2, y: (out - drawH)/2, width: drawW, height: drawH)
