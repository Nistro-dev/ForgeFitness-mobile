...
export class ActivateWithKeyUseCase {
    users;
    keys;
    constructor(users, keys) {
        this.users = users;
        this.keys = keys;
    }
    async exec({ code, deviceId }) {
        ...
        const user = await this.users.findByEmail((await this.users.findByDeviceId(deviceId))?.email ?? '');
        if (!user) {
            throw new AppError('USER_NOT_FOUND', 'USER_NOT_FOUND', 404);
        }
        const token = jwt.sign({ sub: user.id, role: user.role, deviceId }, env.JWT_SECRET, { expiresIn: '180d' });
        return {
            token,
            user: {
                id: user.id,
